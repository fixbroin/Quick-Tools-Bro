...
const storageKey = 'quotation-form-data';
...
export default function QuotationMakerPage() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
        try {
            ...
            toast({ title: 'Draft Loaded', description: `Your saved quotation draft has been loaded.` });
        } catch (error) {
  ...
  const generatePdf = async () => {
    ...
    try {
        ...
        doc.text(`Quotation #: ${formState.docNumber}`, 14, y);
        ...
        autoTable(doc, {
            ...
            head: [[
              ...
              { content: `Rate (${currencySymbol})`, styles: { halign: 'right' as const } },
              { content: `Amount (${currencySymbol})`, styles: { halign: 'right' as const } }
            ]],
