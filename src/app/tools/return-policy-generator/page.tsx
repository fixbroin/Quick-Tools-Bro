...
import { generateReturnPolicy } from '@/ai/flows/return-policy-flow';
...
export default function ReturnPolicyGeneratorPage() {
  ...
  const handleSubmit = async (values: ReturnPolicyInput) => {
    setIsLoading(true);
    setGeneratedPolicy('');
    try {
      const result = await generateReturnPolicy(values);
